...
# ============================================================
SECRET_KEY = "bizsight_ai_secret_key_2024"  # Change this in production!
...
# ============================================================
# CUSTOM CSS - ENHANCED WITH NEW FEATURES
# ============================================================
st.markdown("""
<style>
/* Main Header */
...
def show_auth_page():
    ...
    st.markdown("### Your Business Intelligence Platform")
    st.markdown("---")
...
def show_analytics():
    ...
    st.markdown("<h2 class='section-header'>📈 Business Analytics</h2>", unsafe_allow_html=True)
    ...
    if transactions:
        ...
        with col1:
            st.markdown("### Expense Categories")
            expense_cat = df_trans[df_trans['Type'] == 'expense'].groupby('Category')['Amount'].sum().reset_index()
        ...
        with col2:
            st.markdown("### Sales Categories")
            sales_cat = df_trans[df_trans['Type'] == 'sale'].groupby('Category')['Amount'].sum().reset_index()
        ...
        df_trans['Date'] = pd.to_datetime(df_trans['Date'])
        df_trans['Month'] = df_trans['Date'].dt.to_period('M').astype(str)
        
        monthly_trend = df_trans.groupby(['Month', 'Type'])['Amount'].sum().unstack(fill_value=0).reset_index()
        ...
        st.info("📭 No data available for analytics. Please add transactions first.")
